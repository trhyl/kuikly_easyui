    ...
    js(IR) {
        browser {
            ...
            commonWebpackConfig {
                ...
                devtool =
                    "source-map" // 不使用默认的 eval 执行方式构建出 source-map，而是构建单独的 sourceMap 文件
            }
        }
...
publishing {
    repositories {
        maven {
            ...
            rootProject.properties["mavenUr?"]?.toString()
                ?.let { url = uri(it) }
        }
    }
}
...
android {
    namespace = "com.hy.kuikly_easyui.easyui"
    compileSdk = 34
    defaultConfig {
        minSdk = 21
        targetSdk = 30
