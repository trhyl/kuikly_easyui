...
fun copyLocalJSBundle() {
    ...
    // Input target path, in easyui/outputs/kuikly/js/release/local/nativevue2.zip
    val sourceDir =
        Paths.get(project.buildDir.absolutePath, "distributions", "kotlin2js")
            .toFile()
...
fun copySplitJSBundle() {
    ...
    // Input target path, in easyui/outputs/kuikly/js/release/split/page
    ...
    if (pageDir.exists()) {
        ...
        files?.forEach { file ->
            if (file.isFile) {
                ...
                // Replace development environment JSBundle link with production environment link
                val updatedContent =
                    fileContent.replace(placeText, "page/$fileName")
