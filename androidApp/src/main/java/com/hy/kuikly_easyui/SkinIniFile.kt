...
import com.hy.kuikly_easyui.adapter.execOnSubThread
...
class SkinIniFile(private val context: Context) {
                ...
                context.assets.open(assetsPath).use { inputStream ->
                    inputStream.bufferedReader().use { bufferReader ->
                        ...
                        bufferReader.forEachLine {
                            ...
                            curSectionName =
                                readEachLine(cleanLineStr, curSectionName)
                        }
                    }
                }
                hasLoaded = true
    ...
    private fun readEachLine(
        ...
        if (sectionPattern.matcher(cleanLineStr).matches()) {
            ...
            if (keyValue.size == 2) {
                val section =
                    sections[curSectionName] ?: Section(curSectionName)
